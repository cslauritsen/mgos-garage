...
Device::Device() {
  ...
  std::transform(this->deviceId.begin(), this->deviceId.end(),
                 this->deviceId.begin(),
                 [](unsigned char c) { return std::tolower(c); });

  // Homie setup
  std::string
      ip; // cannot get this until device has fully configured, which is not now
  std::string mac; // cannot get this until device has fully configured, which
                   // is not now

  homieDevice = new homie::Device(
      this->deviceId, std::string(build_version),
      std::string(mgos_sys_config_get_project_name()), ip, mac);
  ...
  //  mgos_sys_config_set_mqtt_ssl_psk_key(homieDevice->getPsk().substr(0,
  //  64).c_str());
  ...
  auto dhtNode = new homie::Node(this->homieDevice, DHT_NODE_NM,
                                 "Temperature/Humidity Sensor", "DHT22");

  auto tempFProp =
      new homie::Property(dhtNode, DHT_PROP_TEMPF, "Temperature in Fahrenheit",
                          homie::FLOAT, false);
  tempFProp->setValue(this->tempf());
  tempFProp->setUnit(homie::DEGREE_SYMBOL + "F");

  auto rhProp = new homie::Property(dhtNode, DHT_PROP_RH, "Relative Humidity",
                                    homie::FLOAT, false);
  rhProp->setUnit("%");

  auto wifiNode =
      new homie::Node(this->homieDevice, WIFI_NODE_NM, "WiFi Station", "Wifi");
  auto rssiProp =
      new homie::Property(wifiNode, WIFI_NODE_RSSI_PROP, "WiFi Signal Strength",
                          homie::INTEGER, false);
  ...
  for (int d = 0; d < doorCount; d++) {
    Door *door = NULL;
    if (d == 0) {
      door = new Door(mgos_sys_config_get_garage_door_a_name(),
                      mgos_sys_config_get_garage_door_a_pin_contact(),
                      mgos_sys_config_get_garage_door_a_pin_activate(), d);
    } else if (d == 1) {
      door = new Door(mgos_sys_config_get_garage_door_b_name(),
                      mgos_sys_config_get_garage_door_b_pin_contact(),
                      mgos_sys_config_get_garage_door_b_pin_activate(), d);
    }
    this->doors.push_back(door);

    auto doorNode = new homie::Node(this->homieDevice, door->getOrdinalName(),
                                    door->getName(), "GarageDoor");
    homieDevice->addNode(doorNode);

    if (door->getActivateRelayPin() >= 0) {
      auto relayProp =
          new homie::Property(doorNode, DOOR_ACTIVATE_PROP, "Activation Button",
                              homie::BOOLEAN, true);
      relayProp->setValue(0);
      doorNode->addProperty(relayProp);
    }
    if (door->getContactPin() >= 0) {
      auto contactProp = new homie::Property(
          doorNode, DOOR_CONTACT_PROP, "Is Door Open?", homie::BOOLEAN, false);
      // contactProp->setFormat("open,closed,unknown"); // for use with
      // homie::ENUM
      contactProp->setValue(kOpen == door->getStatus() ? std::string("true")
                                                       : std::string("false"));
...
void Device::setIpAddr(std::string s) { this->ipAddr = std::string(s); }
...
Door::Door(const char *aName, int aContactPin, int aActivatePin, int index) {
  ...
  LOG(LL_INFO,
      ("Cfg door %s, ctc=%d act=%d ord=%s", this->name.c_str(),
       this->contactPin, this->activateRelayPin, this->ordinalName.c_str()));
}
...
void Door::activate(void) {
  ...
  mgos_set_timer(mgos_sys_config_get_garage_door_activate_millis(), 0,
                 _deactivate_cb, this);
  LOG(LL_INFO,
      ("act door %s pin %d", this->getName().c_str(), this->activateRelayPin));
}
...
std::string Door::getName(void) { return this->name; }
