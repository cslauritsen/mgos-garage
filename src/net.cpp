...
static void network_config_rpc_cb(struct mg_rpc *c, void *cb_arg,
  ...
  char *id = NULL;
  int scan_result =
      json_scanf(result.p, result.len, "{id: %Q, mac: %Q, wifi: {sta_ip: %Q}}",
                 &id, &mac, &ip);
  ...
  if (scan_result < 0) {
    ...
    if (ip) {
      ...
      if (oldVal != str) {
        homieDevice->publish(
            homie::Message(homieDevice->getTopicBase() + "$localip", str));
      }
    } else {
    ...
    if (mac) {
      ...
      if (oldVal != str) {
        homieDevice->publish(
            homie::Message(homieDevice->getTopicBase() + "$mac", str));
      }
    } else {
...
void homie::Device::inquireNetConfig() {
  ...
  mg_rpc_callf(mgos_rpc_get_global(), mg_mk_str("Sys.GetInfo"),
               network_config_rpc_cb, this, &opts, NULL);
  callCount++;
}